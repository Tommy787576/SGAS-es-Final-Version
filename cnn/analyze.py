...
class Analyzer(object):
    ...
    def compute_Hw(self, input_train, target_train, input_valid, target_valid,
                   eta, network_optimizer, unrolled, \
                   normal_candidate_flags, reduce_candidate_flags):
        self.zero_grads(self.model.parameters())
        self.zero_grads(self.model.arch_parameters())
        loss = self.model._loss(input_valid, target_valid)

        # remove the edges with required_grad=False
        requires_grad_arch_parameters = [p for p in self.model.arch_parameters() if p.requires_grad]

        self.hessian = self._hessian(loss, requires_grad_arch_parameters)
        del requires_grad_arch_parameters
        return self.hessian
